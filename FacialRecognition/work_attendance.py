...
def record_attendance(person):
    try:
        ...
        # No need to initialize register_names here, as it will be initialized below
    ...
    if person in register_names:
        # Update the timestamp for the existing person
        for i, line in enumerate(data_list):
            if person == line.split(',')[0]:
                data_list[i] = f'{person},{string_right_now}\n'

        # Write back the updated data to the file
        f.seek(0)
        f.writelines(data_list)
    else:
        # Add a new registration if the person is a newcomer
        f.writelines(f'\n{person},{string_right_now}')

    f.close()  # Close the file after operation
